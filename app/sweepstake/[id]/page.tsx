...
import {
  loadSweepstake, saveSweepstake, loadPlayerId, savePlayerId,
  calculatePoints, distributeTeams, getTeam, formatDate, generateId,
} from "../../../lib/utils";
import { Sweepstake, Player } from "../../../lib/types";
import { POINTS_SYSTEM, TOURNAMENT_AWARDS, TEAMS_2026 } from "../../../lib/data";
...
export default function SweepstakePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      loadSweepstake(id).then((data) => { if (data) setSw(data); });
    }, 10000);
    return () => clearInterval(interval);
  }, [id]);

  async function save(updated: Sweepstake) {
    setSaving(true);
    await saveSweepstake(updated);
    setSw({ ...updated });
    setSaving(false);
  }
  ...
  async function handleJoin() {
    ...
    const newPlayer: Player = {
      id: generateId(), name: joinName.trim(), teams: [], points: 0, joinedAt: Date.now(),
    };
  ...
  function handleResultChange(fixtureId: string, field: "scoreA" | "scoreB", value: string) {
    ...
    const fixtures = sw.fixtures.map((f) =>
      f.id !== fixtureId ? f : { ...f, [field]: value === "" ? undefined : parseInt(value) }
    );
    setSw({ ...sw, fixtures });
  }
  ...
  async function handleKnockoutBonus(teamId: string, round: string, points: number) {
    if (!sw) return;
  ...
  async function handleAward(awardId: string, teamId: string) {
    if (!sw) return;
    const awards = { ...sw.awards, [awardId]: teamId };
    const updated = { ...sw, awards };
    updated.players = calculatePoints(updated);
    await save(updated);
  }

  async function handleSetFixtureTeam(fixtureId: string, side: "teamA" | "teamB", teamId: string) {
    if (!sw) return;
    const fixtures = sw.fixtures.map((f) =>
      f.id !== fixtureId ? f : { ...f, [side]: teamId }
    );
    await save({ ...sw, fixtures });
  }
  ...
  if (loading) return (
    <div className="min-h-screen bg-green-900 flex items-center justify-center text-white">
      <div className="text-center">
        ...
        <p className="text-green-300">Loading sweepstake...</p>
      </div>
    </div>
  );
  ...
  if (notFound) return (
    <div className="min-h-screen bg-green-900 flex items-center justify-center text-white text-center p-8">
      <div>
        ...
        <p className="text-green-300 mt-2">Double check the link and try again.</p>
      ...
      {sw.status === "signup" && !me && (
        <div className="bg-yellow-50 border-b border-yellow-200 px-4 py-4">
          <div className="max-w-2xl mx-auto flex gap-3">
            ...
            <button onClick={handleJoin} disabled={!joinName.trim() || saving}
      ...
      {me && (
        <div className="bg-green-50 border-b border-green-200 px-4 py-2">
          <div className="max-w-2xl mx-auto text-sm text-green-700 font-medium">
            ...
            {sw.status === "active" && <span className="ml-2">· {me.teams.length} teams · {me.points} pts</span>}
            {saving && <span className="ml-2 text-green-500">Saving...</span>}
          </div>
        </div>
      )}
            ...
            {ranked.map((player, i) => (
              <div key={player.id} className={`bg-white rounded-2xl p-4 flex items-center gap-4 shadow-sm border ${player.id === myId ? "border-green-300 bg-green-50" : "border-gray-100"}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-lg ${
                  i === 0 ? "bg-yellow-400 text-yellow-900" : i === 1 ? "bg-gray-300 text-gray-700" :
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {sw.status === "active" && (
                    <div className="text-sm text-gray-500 flex flex-wrap gap-1 mt-0.5">
                      {player.teams.map((tid, idx) => {
                        const t = getTeam(tid);
                        return t ? <span key={tid}>{t.flag} {t.name}{idx < player.teams.length - 1 ? " ·" : ""}</span> : null;
                      })}
                    </div>
                  )}
                </div>
        ...
        {tab === "schedule" && (
          <div className="space-y-2">
            {sw.fixtures.map((fixture) => {
              ...
              const isMine = me?.teams.includes(fixture.teamA) || me?.teams.includes(fixture.teamB);
              const isTba = fixture.teamA === "tba" || fixture.teamB === "tba";
              return (
                <div key={fixture.id} className={`bg-white rounded-2xl p-4 shadow-sm border ${isMine ? "border-green-300" : "border-gray-100"}`}>
                  ...
                  {isTba ? (
                    <div className="text-center text-gray-400 text-sm py-1">Teams to be confirmed</div>
                  ) : (
                  ...
                  {isMine && <div className="mt-2 text-xs text-green-600 font-semibold">⚽ Your team is playing!</div>}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {me.teams.map((tid) => {
                    ...
                    if (!team) return null;
                    const myFixtures = sw.fixtures.filter((f) => f.teamA === tid || f.teamB === tid);
                    const wins = myFixtures.filter((f) => f.played && ((f.teamA === tid && f.result === "homeWin") || (f.teamB === tid && f.result === "awayWin"))).length;
                ...
                {sw.fixtures.filter((f) => !f.played && f.teamA !== "tba" && (me.teams.includes(f.teamA) || me.teams.includes(f.teamB))).slice(0, 5).map((f) => {
                  const tA = getTeam(f.teamA); const tB = getTeam(f.teamB);
          ...
          <div className="space-y-6">
            {sw.status === "signup" && (
              <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                ...
                <p className="text-sm text-gray-500 mb-4">{sw.players.length} players joined. Close sign-up to randomly draw teams.</p>
                <div className="space-y-2 mb-4">
                  {sw.players.map((p) => (
                    <div key={p.id} className="flex items-center gap-2 text-sm text-gray-700">👤 {p.name}</div>
                  ))}
                </div>
                <button onClick={handleCloseSignup} disabled={sw.players.length < 2 || saving}
                  ...
                  {saving ? "Drawing teams..." : "🎲 Close Sign-up & Draw Teams"}
                </button>
            ...
            {sw.status === "active" && (
              <>
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                  <h3 className="font-bold text-gray-900 mb-1">🥅 Set Round of 32 Teams</h3>
                  <p className="text-sm text-gray-500 mb-4">As FIFA confirms each knockout matchup, pick the two teams here. The schedule and "My Teams" tab will update immediately.</p>
                  <div className="space-y-3">
                ...
                <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {sw.fixtures.filter(f => f.teamA !== "tba").map((fixture) => {
                      const tA = getTeam(fixture.teamA); const tB = getTeam(fixture.teamB);
                      if (!tA || !tB) return null;
                      return (
                        <div key={fixture.id} className={`p-3 rounded-xl border ${fixture.played ? "bg-green-50 border-green-200" : "bg-gray-50 border-gray-200"}`}>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <input type="number" min={0} value={fixture.scoreA ?? ""}
                            ...
                            <input type="number" min={0} value={fixture.scoreB ?? ""}
                            ...
                            <button onClick={() => handleSaveResult(fixture.id)}
                              disabled={fixture.scoreA === undefined || fixture.scoreB === undefined || saving}
